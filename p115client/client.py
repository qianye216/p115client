...
class P115Client(P115OpenClient):
    ...
    def extract_file(
        ...
        **request_kwargs, 
        ...
        :param paths:    待解压缩的文件路径（相对于 ``dirname``），如果为空，则是 ``dirname`` 下所有
        ...
        absdir = f"文件/{dirname}" if dirname else "文件"
        data = [
            ("pick_code", pickcode), 
            ("paths", absdir), 
            ("to_pid", to_pid), 
        ...
        def gen_step():
            ...
            if not paths:
                paths = []
                params = {"pickcode": pickcode, "path": dirname}
                while True:
                    ...
                    if next_marker := resp["data"].get("next_marker"):
